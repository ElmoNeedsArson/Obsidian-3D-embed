...
export function applyCameraSettings(cam: THREE.Camera, config: any, controls: OrbitControls, plugin: ThreeJSPlugin) {
    ...
    } else if (plugin.settings.camPosX !== undefined && plugin.settings.camPosY !== undefined && plugin.settings.camPosZ !== undefined && config.camera?.camPosXYZ == undefined) {
        cam.position.x = plugin.settings.camPosX;
        cam.position.y = plugin.settings.camPosY;
        cam.position.z = plugin.settings.camPosZ;
    } else  {
        cam.position.x = 0
        cam.position.y = 5
        cam.position.z = 10
